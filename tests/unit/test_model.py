...
class TestPVModelChain:
    ...
    def test_init_pv_system_wrong_inverter(self, basic_config):
        """Test the init_pv_system function with wrong inverter model."""
        conf_dict = basic_config[0].copy()
        conf_dict["inverter"] = "wrong_inverter"
        with pytest.raises(KeyError, match=f"Device {conf_dict['inverter']} not found in the database."):
            PVSystemManager([conf_dict], *self.location, self.altitude)

    def test_aggregate_model_results(self, pv_sys_mngr: PVSystemManager, weather_df):
